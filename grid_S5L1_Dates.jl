...
function parse_commandline()
    ...
    @add_arg_table! s begin
        "--Dict"
            ...
            arg_type = String
            default = "jsonFiles/S5_radiance_band2_dict.json"
        "--outFile", "-o"
            help = "output filename (default S5_rad_map.nc)"
            arg_type = String
            default = "S5_rad_map.nc"
        "--monthly"
...
## Old function, still used in L2 though...
...
function getNC_var(fin, path, DD::Bool)
    loc = split(path ,r"/")    
    # --------------------------------------
    # Case 1: variable is in root group
    # --------------------------------------
    if length(loc) == 1
        var = fin[path].var
    elseif length(loc)>1
        # --------------------------------------
        # Case 2: navigate into group hierarchy
        # --------------------------------------
        gr = fin.group[loc[1]]
    ...
    # ---------------------------------------------------
    # NEW: detect and average over spectral_channel dim
    # ---------------------------------------------------
    dimnames = NCDatasets.dimnames(var)
    #println(dimnames)

    if "spectral_channel" in dimnames
        #println("Detected spectral_channel dimension, averaging a few points in the middle")
        sc_dim = findfirst(==("spectral_channel"), dimnames)
        nsc = size(var, sc_dim)

        # choose +/-5 around the middle spectral points ## maybe it is faster to compute the entire band. something to test later
        mid = max(1, Int(floor(nsc / 2 - 5))) : min(nsc, Int(ceil(nsc / 2 + 5)))

        # Build an index tuple, where the range of points is at the position sc_dim, and ":" for others
        indices_to_average = ntuple(i -> i == sc_dim ? mid : Colon(), length(dimnames))
        
        # average over the selected spectral channels
        avg_data = mean(var[indices_to_average...], dims=sc_dim) # this takes ~30 min on 87 files

        ## speed test to average the full band:
        # avg_data = mean(var, dims=sc_dim) # this takes ~40 min on 87 files


        # remove collapsed spectral_channel dim
        var_data = dropdims(avg_data, dims=sc_dim)
    else
        var_data = var
    end
    ...
    # ---------------------------------------------------
    # DD handling (original footprint logic)
    # ---------------------------------------------------
    if DD
        si = size(gr[loc[end]])
    ...
    # ---------------------------------------------------
    # Normal flattening behaviour
    # ---------------------------------------------------
    return reshape(var_data[:], prod(si))
end
...
function main()
    ...
    # Create output file:
    dsOut = Dataset(ar["outFile"],"c") # "c" = create new file
    defDim(dsOut,"lat",length(lat))
    defDim(dsOut,"lon",length(lon))
    defDim(dsOut,"time", cT)
    dsLat = defVar(dsOut,"lat",Float32,("lat",), 
        attrib = Dict(
            "units" => "degrees_north",
            "long_name" => "Latitude")
        )
    dsLon = defVar(dsOut,"lon",Float32,("lon",), 
        attrib = Dict(
            "units" => "degrees_east",
            "long_name" => "Longitude")
        )
    dsTime= defVar(dsOut,"time",Float32,("time",),
        attrib = Dict(
    ...
    for (key, value) in dGrid
        ...
        if ar["compSTD"]
            key2 = key*"_std"
            NCDict[key2] = defVar(dsOut, key2, Float32,("time", "lon", "lat"), deflatelevel=4, fillvalue=-999, attrib = Dict("comment" => "Standard Deviation from data"))
        end
    end
    println(" ")
